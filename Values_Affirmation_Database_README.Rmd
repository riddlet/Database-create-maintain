...
##Essays
```{r, echo=FALSE}
df.essays <- read.csv('../Data/3 CSV Files/essays10.12.15.csv', sep='|', quote="")
...
levels(temp$Condition) <- c(rep('Other', 3), rep('Control', 7), 
                            rep('Treat', 7))
temp$prompt.factor <- factor(temp$Essay.Prompt, levels=letters)
#lvls <- c(LETTERS, paste('A', LETTERS, sep=''))
#temp$prompt.factor <- factor(temp$Essay.Prompt, labels=lvls[1:num.prompts])
#temp$seq <- with(temp, ave(prompt.factor, Intervention_number, Cohort, FUN=seq_along))

#prompt.wide <- dcast(temp, Intervention_number~Cohort, value.var = 'prompt.factor')

#temp2 <- temp[which(Condition=='Control'),]
